...
## 14. Checkpoint 3：2026-09-25 T15 + T17 发布集成

- Integration branch：`codex/t15-t17-release-integration`，以 T17 `e580595a0b47023020ec1bef9652838adaae5e66` 为基线合入 T15；`strings.xml` 自动合并，无冲突标记，播放器代码与 T17 启动器主题代码没有覆盖关系。
- Verification：`testMobileArm64_v8aDebugUnitTest --tests com.fongmi.android.tv.player.autoskip.AutoSkipChapterPolicyTest`、`compileMobileArm64_v8aDebugJavaWithJavac`、`compileLeanbackArm64_v8aDebugJavaWithJavac` 同次执行成功，`BUILD SUCCESSFUL in 2m 32s`。
- Preserved：T17 H3/木纹/玻璃/荷叶/大理石/水面主题、启动过场和长来源标签两行约束；T15 手工片头片尾优先级、版本哈希、本地确认规则与 TTL/LRU 均保留。
- Remaining device gate：发布签名 Beta 后覆盖安装电视，确认新版版本信息、启动过场和普通播放不回归；具名章节候选仍需找到实际带 OP/ED 章节的媒体完成行为验收。
